...
class FaceFeaturesExtractionService {
  ...
  Future<List<List<double>>> extractFaceFeaturesAsync(
    cv.Mat image,
    cv.Mat faces,
  ) async {
  ...
  /// extractFaceFeaturesSync

  /// Extracts facial features from the input face boundary and returns
  /// a list of detected facial features.
  List<List<double>> extractFaceFeatures(cv.Mat image, cv.Mat faces) {
    List<List<double>> faceFeatures = [];
    final faceBoundaries = List.generate(faces.rows, (i) {
      final x = faces.at<double>(i, 0).toInt();
      final y = faces.at<double>(i, 1).toInt();
